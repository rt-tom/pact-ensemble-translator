...
try {
    ...
    Assert-True ($runner -match '(?s)Stage protocol REUSED: \$Label(?:(?!return).)*Write-MonitorState -Stage \$Label -Status ''REUSED''(?:(?!return).)*return') 'A reusable authoritative aggregate must complete without rerunning its model stage.'
    Assert-False ($runner -match '(?s)Stage protocol REUSED: \$Label(?:(?!return).)*Invoke-PythonStage') 'A reusable aggregate must not make hidden model HTTP calls.'
