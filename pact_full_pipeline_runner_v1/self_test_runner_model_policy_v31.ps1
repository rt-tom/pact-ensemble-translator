...
try {
    ...
    Assert-True ($runner -match '(?s)Stage protocol REUSED: \$Label(?:(?!return).)*Start-LlamaServer \$Profile(?:(?!return).)*Invoke-PythonStage -Label \$Label -Arguments \$Arguments -Outcome ''REUSED''') 'Aggregate reuse must preserve the owned server for the producer cache-identity check.'
