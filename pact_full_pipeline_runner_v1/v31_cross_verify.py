...
Если decision=repair, repair_scope должен быть span, sentence или paragraph.
Если decision=keep или uncertain, используй repair_scope="span" как
нейтральное schema-compatible значение; не используй "none".
...
def parse(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if scope == "none":
        if decision == "repair":
            raise ValueError(
                "repair_scope must be span, sentence, or paragraph "
                "when decision is repair"
            )
        scope = "span"
