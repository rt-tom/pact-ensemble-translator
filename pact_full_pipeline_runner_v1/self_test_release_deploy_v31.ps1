...
function Get-IndependentGitBlobSha256([string]$Root, [string]$Commit, [string]$RelativePath) {
    $output = Join-Path $Root ("git-blob-" + [guid]::NewGuid() + '.bin')
    try {
        # cmd.exe redirects the native git stdout directly to a file. No
        # PowerShell text pipeline is involved in this independent oracle.
        $object = "${Commit}:$RelativePath"
        $command = 'git -C "' + $Root + '" cat-file blob "' + $object + '" > "' + $output + '"'
        & $env:ComSpec /d /s /c $command
        if ($LASTEXITCODE -ne 0) { throw "Independent git cat-file failed for $RelativePath" }
        $sha256 = [Security.Cryptography.SHA256]::Create()
        try { return (($sha256.ComputeHash([IO.File]::ReadAllBytes($output)) | ForEach-Object { $_.ToString('x2') }) -join '') }
...
try {
    ...
    $blobBytes = [ordered]@{
        'utf8-no-bom.txt' = [Text.UTF8Encoding]::new($false).GetBytes("Привет`n")
        'utf8-with-bom.txt' = [byte[]](@(0xEF, 0xBB, 0xBF) + [Text.UTF8Encoding]::new($false).GetBytes("Привет`n"))
        'lf-blob.txt' = [Text.Encoding]::ASCII.GetBytes("one`ntwo`n")
        'crlf-blob.txt' = [Text.Encoding]::ASCII.GetBytes("one`r`ntwo`r`n")
        'binary-blob.bin' = [byte[]](0, 255, 1, 13, 10, 128, 0, 42)
        'path with spaces/binary blob.bin' = [byte[]](42, 0, 255, 13, 10)
    }
    foreach ($relative in $blobBytes.Keys) {
        $path = Join-Path $temp $relative
        New-Item -ItemType Directory -Force -Path (Split-Path -Parent $path) | Out-Null
        [IO.File]::WriteAllBytes($path, [byte[]]$blobBytes[$relative])
    }
    ...
    $targetCommit = (& git -C $temp rev-parse 'v3.1.3-test^{commit}').Trim()
    foreach ($relative in $blobBytes.Keys) {
        $actual = @($manifestObject.files | Where-Object { $_.path -eq $relative })
        Assert-V31Release ($actual.Count -eq 1) "Manifest lacks blob fixture: $relative"
        $expected = Get-IndependentGitBlobSha256 -Root $temp -Commit $targetCommit -RelativePath $relative
        Assert-V31Release ($actual[0].sha256 -eq $expected) "Git blob SHA-256 mismatch: $relative"
    }
    $lfHash = @($manifestObject.files | Where-Object { $_.path -eq 'lf-blob.txt' })[0].sha256
    $crlfHash = @($manifestObject.files | Where-Object { $_.path -eq 'crlf-blob.txt' })[0].sha256
    Assert-V31Release ($lfHash -ne $crlfHash) 'LF and CRLF blobs must retain distinct SHA-256 values.'
    & $script -ProjectRoot $temp -ReleaseRef v3.1.3-test -ManifestPath $manifest | Out-Null
    ...
    $cacheFile = Join-Path $temp 'pipeline_runs\cache-preservation.bin'
    New-Item -ItemType Directory -Force -Path (Split-Path -Parent $cacheFile) | Out-Null
    [IO.File]::WriteAllBytes($cacheFile, [byte[]](1, 2, 3, 255))
    ...
    Assert-V31Release (([Security.Cryptography.SHA256]::Create().ComputeHash([IO.File]::ReadAllBytes($cacheFile)) | ForEach-Object { $_.ToString('x2') }) -join '' -eq '3e6f9aae16382bf563d8991b6da1b92213911f0dd5deea3ecaccf2f35a56794a') 'Deploy changed cache data.'
