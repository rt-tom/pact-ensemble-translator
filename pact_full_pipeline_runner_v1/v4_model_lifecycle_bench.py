...
* No production chapter/config is touched, and this script still runs no
  translation pipeline. It does import ``LifecycleAdapter`` from
  ``pact_v4.runtime.model_lifecycle`` (promoted there for
  `V4_STRICT_DRIVER_CHAPTER_TRIAL_TASK_RU.md` so the strict driver and
  this bench share one validated implementation instead of drifting
  copies) -- that is the one ``pact_v4`` import this script has, and it
  is the lifecycle mechanics only, not a gate/driver import.
* The lifecycle adapter only starts/stops the ``llama-server`` process it
  itself launched. It refuses to attach to or kill a server it did not
  start (probes the port first; aborts if something already answers
  there).
...
from pact_v4.runtime.model_lifecycle import LifecycleAdapter, LifecycleError
...
# --------------------------------------------------------------------------
# Raw /completion helper for the synthetic role prompts (LifecycleAdapter
# itself, imported from pact_v4.runtime.model_lifecycle, only owns
# process start/stop/VRAM -- it has no opinion about the completion
# endpoint shape, so that stays local to this bench script).
# --------------------------------------------------------------------------

def raw_completion(base_url: str, prompt: str, n_predict: int) -> dict:
    payload = {
        "prompt": prompt,
        "n_predict": n_predict,
    ...
    text_parts: list[str] = []
    server_timings = None
    with requests.post(f"{base_url}/completion", json=payload, stream=True, timeout=120) as resp:
    ...
    if first_token_seconds is None:
        raise LifecycleError("raw_completion() received no streamed tokens.")
    return {
...
def run_benchmark(n_switches: int, host: str, port: int,
    ...
    adapter = LifecycleAdapter(
        exe, device, host, port, log_dir, MODELS,
        startup_timeout=startup_timeout, unload_timeout=unload_timeout,
    )
        ...
        try:
            for seg in plan:
                ...
                try:
                    ...
                    result = raw_completion(adapter.base_url, prompt_spec["prompt"], prompt_spec["n_predict"])
