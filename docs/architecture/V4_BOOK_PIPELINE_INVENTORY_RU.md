...
|---|---|---|---|
| `generator` (`balanced_literary`) | Перевод целой главы одним вызовом | `opencode/muse-spark-1.2-contributor-free` | `pact-v4-prompt-balanced-literary/v7` (`pact_v4/phase2/prompts.py`) |
...
version `pact-v4-prompt-balanced-literary/v7`; рендер через `render_prompt`).
