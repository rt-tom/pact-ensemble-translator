...
3. **Cap на findings, не на вызовы** (решение владельца 2026-08-08, правка 2026-08-11 после run_010): max 100 eligible findings на главу ремонтируются (дефолт 100, конфигурируемо через `findings_cap`); сверх лимита → debt с пометкой «policy_limit: repair_findings_cap_100» (аналог remote_budget); после прогона зафиксировать, сколько ушло в debt, для будущей калибровки. Прежний cap 10 резал 73% находок (run_010: 43 issues, 37 eligible → 27 в debt, включая реальные TP) — решение владельца 2026-08-11: НЕ экономить токены на repair, cap не должен резать TP. Микробатчи (3-4, Cheng) при этом ОСТАЮТСЯ: они про качество батча, не про бюджет.
...
6. Lifecycle-политика (policy qualification, а не независимое доказательство): `changed_fact`/числовые/glossary findings требуют **детерминированного подтверждения кодом**; остальные категории eligible для verification при `confidence=high` (severity НЕ является eligibility-фильтром — у Qwen severity некалибрована: реальные ошибки идут minor, стилистика major; ревью out-of-sample 2026-08-10); Qwen finding = кандидат, не доказательство; Qwen-only семантические находки ниже порога → debt/diagnostic, не авто-ремонт; malformed/empty → fail-closed; когда `complete`, когда `accepted_degraded`.
