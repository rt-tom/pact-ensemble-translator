...
## Результат измерения 2: стоимость lifecycle (Gemma ⇄ Qwen, SYCL, 2026-07-31)

Задача 2 из «Что измерить до кода» — отдельный синтетический
`Gemma -> Qwen -> Gemma -> …` lifecycle-benchmark, ранее прямо названный
недостающим в разделе «Как использовать текущий двухфазный sequential
run». Инструмент: `pact_full_pipeline_runner_v1/v4_model_lifecycle_bench.py`
(standalone, без импорта `pact_v4`, не трогает production config/артефакты;
lifecycle adapter управляет только процессом, который сам запустил, и
подтверждает освобождение VRAM опросом Windows-счётчика
`GPU Process Memory\Dedicated Usage` по PID, а не HTTP-статусом).

**Два билда, не одно измерение.** Изначально планировалось сравнить Vulkan
(`C:\llama-new-vulkan`) и SYCL (`C:\llama-sycl`, тот же commit `a2be61dc8`,
что и vulkan-билд, для честного сравнения). Смоук-тест на Vulkan (2
переключения) вскрыл `vk::Queue::submit: ErrorDeviceLost` при загрузке
Qwen сразу после подтверждённого освобождения VRAM от Gemma — при этом та
же модель грузится штатно как отдельный процесс. Пользователь параллельно
провёл собственное ручное сравнение и выбрал SYCL (`C:\llama-sycl-new`,
более новый билд, `version 10205`) как решительно более быстрый на чтении
и генерации на этом железе; Vulkan-ветка задачи закрыта этим решением, не
доведена до 20-switch прогона. Полная запись содержит только SYCL.

**Профиль запуска** — production-профиль пользователя, не облегчённый
синтетический (после итерации): Gemma — `--model-draft` (MTP), `-ngl 99
-ncmoe 18 --load-mode mmap --reasoning-budget 0 -c 32768 -fa on --jinja`;
Qwen — `-fit on -fitt 1280 -b 2048 -ub 512 -ctk q8_0 -ctv q8_0 -c 32768`
(симметрично `--load-mode mmap`, `SYCL0`, тот же `-c`). Три разные по форме
synthetic-роли (не переименованный один и тот же запрос): Gemma
generation-подобный (128 токенов продолжения), Qwen gate-подобный (48
токенов JSON-вердикта), Gemma preference-подобный (A/B выбор,
`temperature=0`, фиксированный seed).

**Прогон** (`gate_bench_runs/v4_model_lifecycle_bench/sycl_001/measurement_record.json`):
20 переключений (21 загрузка модели — 1 startup + 20 restart, точно число
из раздела «Подсчёт перезапусков» для 10 high-risk chunk'ов у строгого
варианта), **20/20 успешно, 0 ошибок, 0 retry**, `wall_clock_seconds =
643.1` (~10.7 мин).

| Роль | n | cold_acquire median / p95 (s) | unload median / p95 (s) | first_token median (s) | completion median (s) | peak_vram median (MB) |
|---|---:|---:|---:|---:|---:|---:|
| Qwen gate | 10 | 17.58 / 19.30 | 5.92 / 6.28 | 2.19 | 3.76 | 9579 |
| Gemma preference | 10 | 12.94 / 17.95 | 6.05 / 6.23 | 1.79 | 1.88 | 10248 |
| Gemma generation (n=1, initial) | 1 | 12.39 | — | 2.26 | 10.60 | 10248 |

`unload_seconds` включает намеренную 3-секундную "driver settle" паузу
после подтверждённого нулевого `Dedicated Usage` (добавлена после того же
Vulkan device-lost инцидента как предохранитель; на SYCL ни разу не
понадобилась как защита от сбоя, но включена в замер как часть реальной
стоимости чистого перехода, а не скрыта) — то есть подтверждённое
опросом освобождение VRAM само по себе укладывается в ~3s из
наблюдаемых ~6s.

**Значение для выбора топологии.** На этом железе/билде строгий
stop-and-switch на 10 high-risk chunk'ов (20 restart) стоит ≈ `10×17.58 +
10×12.94 + 20×~6.0 ≈ 425s` чистого lifecycle-overhead (cold_acquire +
unload, без учёта времени самой generation/gate/preference) — то есть
average ≈21s на restart, не десятки секунд/минут, которые оправдывали бы
approximation-варианты по одной лишь стоимости restart. Это не решает
выбор топологии само по себе (качество/discourse — по-прежнему открытый
вопрос из раздела «Открытый вопрос для review»), но снимает
неопределённость именно по cost-стороне: на SYCL0/Arc B580 c этим
профилем restart дешевле, чем предполагала любая догадка "20 restart
недопустимо дорого" без числа.

**Ограничения этого измерения:**

1. `n=1` для Gemma generation-роли (только initial startup); все 10
   Gemma-сегментов после restart были размечены как `preference`
   (короткий A/B-промпт), а не чередовались generation/preference — то
   есть per-restart Gemma-стоимость здесь измерена на коротком
   preference-запросе, не на generation-подобной нагрузке текущего чанка.
   Реальный chapter run тратит на Gemma restart дольше из-за более
   длинной generation-completion (см. `completion_seconds=10.6s` у
   единственного generation-сэмпла против `~1.9s` у preference).
2. Vulkan-ветка не доведена до сопоставимого 20-switch прогона — сравнение
   backend'ов, изначально запланированное этой карточкой, не завершено;
   решение "SYCL быстрее" основано на ручном тесте пользователя вне этого
   инструмента, не на replicated measurement record.
3. Один прогон (`n=1` series) на одной машине — variance между прогонами
   (thermal throttling, фоновая нагрузка ОС) не измерена.
