...
## 6a. Прогон на локальных моделях для сравнения качества (владелец, 2026-08-07)

Отдельная задача ПОСЛЕ валидационного прогона eff-a1a2 (remote). Цель — не
экономия, а **сравнение качества перевода**: тот же book-run (те же главы
0001+0002, тот же chunk_plan/snapshot), но на локальных моделях (Qwen/Gemma
локально через llama-server). Ожидаемо займёт больше времени, чем remote.

- Не является A/B-сравнением v3/v4 (запрет владельца на сравнения версий не
  касается этого: сравниваются МОДЕЛИ перевода, а не архитектуры пайплайна).
- Новый `--out-base` каталог (например, `v4_book_0001-0002_local_*`), не
  перезаписывать remote-артефакты.
- Запуск — только владельцем вручную вне чата (правило 2026-08-06).
- После прогона — сравнение качества по финальным `translations.json` обеих
  глав (remote vs local): та же методология, что в A1b-исследовании
  (инвентаризация артефактов/типографики, человеческая оценка литературности
  на выборке PID), плюс расход (usage.ndjson: calls/cost/time) как контекст.
- Решение о том, какая модель — production, принимает владелец после
  сравнения; это НЕ подразумевает автоматической смены конфигурации.
