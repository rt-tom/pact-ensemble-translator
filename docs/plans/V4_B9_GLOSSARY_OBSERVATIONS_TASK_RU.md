...
### 3. Политика наблюдений — решение владельца

- **Вариант A (shadow-only, рекомендуется)**: кандидаты копятся в
  `observations.json`, promote переносит их как обычные наблюдения (существующая
  логика B7); человек ревьюит перед промоушеном вручную.
- **Вариант B (авто-промоут)**: кандидаты с высокой частотой и консенсусом
  автоматически промоутятся в glossary при complete. Риск: плохой target без
  человеческой проверки.
- По умолчанию в карточке: Вариант A; окончательно — решение владельца.
...
- Integration: book-run двух глав — вторая видит обновлённый glossary после
  promote первой (если Вариант B) или observations накоплены (Вариант A).
...
5. DECISIONS.md — запись о политике наблюдений (Вариант A/B) в том же коммите.
...
Не меняй phase1/2, cascade, risk; v3 не трогай. Политика наблюдений — по решению
владельца (по умолчанию shadow-only, Вариант A).
```
