...
- Рекомендации:
  ...
  (д) **согласование рода рассказчика** — см. раздел 7.

## 7. Анализ смены рода рассказчика в chunk0015–chunk0016

### 7.1 Факт

Рассказчик — мужчина (chunk0002: «Я дошел», «Я вздохнул», «Я уехал три года назад»; fidelity-гейт при карантине chunk0005: «gender agreement error in p00095 (feminine verb for the male narrator)»). Смена рода начинается в chunk0015 (p00366: «я не увидела» — женский) и полностью захватывает chunk0016 (p00382–p00400: «я сидела», «Я встала», «Я побежала», «я ответила»). Самое показательное — зеркальная фраза: p00381 «что только что **увидел**» (мужской, обращено к рассказчику в конце chunk0015) → p00386 «что только что **увидела**» (женский, та же фраза в chunk0016). При этом p00381 (мужской) говорит другому персонажу «Не пытайся отмахнуться от того, что только что увидел» — подтверждая мужской род адресата. Оба чанка (15 и 16) — committed, прошли все гейты; `translations.json` и `repair_report` включают chunk0016 как прошедший.

### 7.2 Почему ни один слой не поймал

| Слой | Что проверяет | Мог бы поймать? |
|---|---|---|
| **Генерация** | source + left_context (рус. перевод chunk0015, мужской) | gemma видел мужской контекст, но нет ограничения на род → игнорирует |
| **Qwen fidelity гейт** | верность АНГЛИЙСКОМУ источнику | английский gender-neutral («I», «you saw»); гейт доказал способность ловить род (p00095 — внутреннее противоречие в пределах чанка), но chunk0016 женский **сам по себе** не противоречит ни источнику, ни самому себе — межчанковая граница не в scope |
| **Детерминированный integrity** | numbers / mixed_script / glossary / missing | структурно не умеет |
| **Step 6 аудит** (qwen_chapter_audit, gemma_russian_review) | calque/scene/omission/addition/dialogue/referent/register/ty_vy/repetition | у chunk0016 7 находок (scene×2, omission, calque×3, register) — категория «narrator gender» отсутствует |
| **Step 7 repair** | только по флагнутым находкам | ничего не флагнуто → не чинится |
| **Step 8** | deterministic + узкий qwen smoke | smoke — не quality-гейт, срабатывает только при изменении вне re-audit |
| **Форматирование** | wrap-only | не трогает текст |

**Ни на одном этапе нет инварианта согласования рода рассказчика.** Теоретически поймать могли бы ровно два места, где модель видит контекст:
1. **Qwen fidelity гейт** — он доказал, что умеет рассуждать о роде рассказчика (p00095), но его scope — верность источнику текущего чанка, а не межчанковая консистентность.
2. **qwen_chapter_audit** (оконный по главе) — но его рубрика не включает род, и он не сработал.

Всё остальное (детерминированный integrity, formatting) структурно не способно.

### 7.3 Почему библия книги не помогла

Она **никогда не попадала в модель**:
- `snapshot_factory.py` читает `book_memory.json` только для `book_memory_hash` (identity-хеш); сама проза библии (описания персонажей, пол рассказчика) в промпт не попадает.
- `_glossary_entries(memory)` извлекает только `memory.glossary` (имена/термины), не `memory.book_memory`.
- В `prompts.py` промпт говорит «frozen book/chapter memory snapshot (glossary and style/voice constraints)», но реально рендерит только `GLOSSARY` + `STYLE_VOICE_CONSTRAINTS` (= `{}`, т.к. strict runner передаёт `style_constraints={}`).
- В `cascade.py` (fidelity гейт), в аудите и в ремонте ссылок на memory нет вообще.

То есть даже если бы в библии был указан пол рассказчика — ни генератор, ни гейт, ни аудит его не видят.

### 7.4 Рекомендации по исправлению

1. **Подавать библию/канонический пол рассказчика в fidelity-гейт и аудит** как явное ограничение (из библии или из устоявшейся левой части главы).
2. **Добавить категорию «narrator_gender» в qwen_chapter_audit**, работающую по всей собранной главе, а не по чанку: сканировать финальный текст на самоотсылочные формы «я + глагол прошедшего времени» и сверять род с установленным.
3. **Детерминированный вариант**: regex-скан финального текста по всей главе на паттерн `я\s+\S+` + past-tense verb gender mismatch (уже реализованный `find_mixed_script` может быть расширен на narrator gender).
