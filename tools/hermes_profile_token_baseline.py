...
(docs/plans or .hermes/plans/2026-08-06_221320-hermes-profile-token-efficiency.md).
...
# Never touch these columns — they may carry private content.
_BANNED_COLUMNS = frozenset(
    {
        "content",
        "system_prompt",
        "origin_json",
...
def _pct(sorted_vals, p):
    if not sorted_vals:
        return None
    n = len(sorted_vals)
    idx = min(n - 1, max(0, int(p / 100.0 * n)))
...
def _safe_columns(table: str, cur) -> list[str]:
    cols = [r[1] for r in cur.execute(f"PRAGMA table_info({table})").fetchall()]
    return [c for c in cols if c not in _BANNED_COLUMNS]


def _session_summary(cur, cols):
    """Per-session aggregates from `sessions` (all columns safe)."""
    sel = ", ".join(cols)
...
def _message_summary(cur):
    """Aggregate-only message stats. Never selects content columns."""
    roles = cur.execute(
...
def analyze_profile(profile_dir: Path) -> dict:
    ...
    if not db.exists():
        return {"error": f"state.db not found: {db}"}
    cfg = profile_dir / "config.yaml"
    fingerprint = {
        "state_db_sha256": hashlib.sha256(db.read_bytes()).hexdigest(),
        "state_db_bytes": db.stat().st_size,
        "config_yaml_sha256": (
    ...
    cur = con.cursor()
    try:
        sess_cols = _safe_columns("sessions", cur)
        use_cols = _safe_columns("session_model_usage", cur)
        sessions = _session_summary(cur, sess_cols)
        usage = _usage_summary(cur, use_cols)
