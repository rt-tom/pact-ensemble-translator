...
def test_payload_captures_all_sampling_fields(tmp_path):
    ...
    # Build synthetic policies for translator and reviewer
    from pact_v4.runtime.runtime_config import TRANSLATOR_ROLES, REVIEWER_ROLES
    # Check sampling_for_role returns all fields
    ...
    # Check that a BackendModelCaller serializes all fields
    ...
    # Create synthetic policy for generator
    ...
    # Need to trigger a call via the adapter's __call__ but we can directly test the request construction by inspecting backend
    # Instead, test that policy's sampling is correctly used in CompletionRequest via the adapter's internal logic
    # We do a simple check: sampling_for_role returns all fields, and per_role_hash differs when sampling changes
