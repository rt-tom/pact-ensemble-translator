...
- **Simple CLI is profile-selecting, not profile-editing.** `--local` selects the canonical local profile. Bare `--remote` selects the canonical remote profile and uses its defaults; `--remote translator/reviewer` overrides those two role groups. Defaults are Muse Free translator/repair, Luna standard reviewer roles, reasoning 3, and managed server. A provider-qualified compatibility form remains accepted where the registry requires it. Existing explicit `--runtime-config` is advanced mode. Numeric `N` is a single-chapter shorthand for `N-N`; `--whole-chapter` is injected for every book run; incompatible explicit topology choices fail rather than silently override the simple policy.

- **Scope-A state sync remains the launcher integration.** Every simple book mode, local or remote, injects media book id `1`, target `media-snap`, root `/home/rt/pact_runs`, with explicit book-id override. Therefore an RT local run fetches current media state before `MemoryManager` initialization and publishes accepted updates afterward, just as an RT remote run does. RT uses the restricted SSH facade. Media selects a local facade adapter offering the same restricted operations and validation, avoiding a self-SSH dependency. The adapter produces machine-readable acceptance/rejection data; launcher output summarizes the final publication outcome and returns non-zero on rejection/missing confirmation.
