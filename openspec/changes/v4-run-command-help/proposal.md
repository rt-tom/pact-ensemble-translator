...
- Define and implement a simplified, supported book-first v4 command over the existing production CLI: an explicit chapter range plus a run-config file, while retaining a chapter mode.
- Resolve the range through the existing `v4_book_run` chapter HTML pattern; automatically create an output subdirectory under `D:\pact\gate_bench_runs` named `book_0027-0032_local|remote_<timestamp>`.
- Make `--help` complete, grouped, and truthful for range/source resolution, output naming, runtime configuration, topology, resume, audit/formatting/markup behavior, and safety/owner-run boundaries.
...
- No prompts, translation/audit/formatting/tag semantics, model routing, persistent artifact schema, or production pipeline run is changed by this proposal.
