...
**D8. Модель/ретрай/фейл — reuse reviewer роли (без отдельного бюджета).** Отдельная роль НЕ заводится; резолвер полностью переиспользует `reviewer` транспорт: primary `russian_selector`, fallback `fidelity_reviewer`, далее `qwen_audit` для `local`; если ни одного binding нет — `fail-closed`. Наследует `temperature 0`/`seed`/`reasoning`/`structured_output prompt_only` и `max_output_tokens` reviewer'а как есть (его бюджет уже покрывает `reasoning 8192 + content` для аудита `~3.6k` входа; для `glossary` батча `~1k` content этого достаточно с запасом, отдельный `3072` не вводится — `3072` был лишь content-оценкой без учёта `reasoning` и привёл бы к `truncated`). `response_schema=glossary_proposal/v1` (strict). `1 logical batch` = `≤3` transport attempts; `≤1` batch/главу при непустом сете (`off`/пусто/валидный sidecar → `0`). `Failure` → `LOG warning`, sidecar не пишется, глава не падает, `promotion fail-closed`. Каждый `attempt` — в `usage.ndjson` (`glossary_resolver`), `backend events`, `phase_progress`.
