...
- [ ] 2.1 Реализовать `GlossaryResolver` на `reviewer` транспорте (`russian_selector` primary, fallback `fidelity_reviewer` → `local qwen_audit / remote Luna`, bounded output `min(reviewer_budget,3072)` / override `3072`, `prompt_only`, `response_schema glossary_proposal/v1`) — верификация: юнит-тест схемы, `3` главы без truncation
