...
- [x] 1.1 Extend `ALLOWED_MODEL_REQUEST_FIELDS` + `_validate_model_request` with `repeat_penalty/repeat_last_n/frequency_penalty/presence_penalty` (type/range, forbid `max_output_tokens`/`reasoning_budget` still).
- [x] 1.2 Extend `role_budgets` schema with optional `reasoning_budget: int 0..8192`; validate, fail-closed on bad type/range.
