...
- Раннер: единый резолв `--book <slug>` (неуказанный slug = `pact` только для обратной совместимости); утверждённый `chapters.json` — единственный авторитет для заголовков (отдельно от `glossary.json`) и POV. `arc_names.json` перестаёт быть runtime-входом; блок `CHAPTERS:` выводится single-derivation из записей `chapters.json` (alternative B, owner decision; byte-regression не требуется, B4 waived), пустая карта — без блока.
