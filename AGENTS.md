...
- **Аналитические задачи (оценка/отчёт/исследование) — сходимость ревью
  (правило владельца 2026-08-07)**: такие задачи («просто оценить данные
  прогона», baseline, audit) НЕ должны уходить в бесконечные циклы ревью.
  Ревьюер проверяет заявленный scope и корректность цифр/выводов; НЕ расширяет
  требования новыми пунктами в каждом раунде (анти-паттерн: 3 раунда
  замечаний по задаче-оценке). Артефакт старой базы ветки (дифф «удаляет»
  чужие коммиты, например монитор) — это НЕ блокер содержания: отдельным
  пунктом «перебазировать ветку перед мержем», а не CHANGES REQUESTED по
  контенту. Если новый раунд не приближает к approve — архитектор вмешивается:
  собирает оставшиеся замечания в один список, передаёт developer одной
  итерацией и ограничивает ревью скоупом карточки.
